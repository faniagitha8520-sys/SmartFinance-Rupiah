...
function HutangView({ S, c, tx, cardStyle, labelStyle }) {
  ...
  const piutangByPerson = {};
  piutangAll.forEach(t => {
    const key = t.catatan || t.item || "Lainnya";
    if (!piutangByPerson[key]) piutangByPerson[key] = [];
    piutangByPerson[key].push(t);
  });
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={cardStyle}>
        <div style={{...labelStyle,marginBottom:12}}>📉 HUTANG (Saya berhutang ke orang lain)</div>
      ...
      <div style={cardStyle}>
        <div style={{...labelStyle,marginBottom:12}}>💳 PIUTANG (Orang lain berhutang ke saya)</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:16}}>
        ...
        <div style={{marginTop:4}}>
          ...
          {Object.keys(piutangByPerson).length === 0 && <div style={{color:S.textMuted,fontSize:12}}>Tidak ada piutang tercatat</div>}
        </div>
      </div>
    </div>
  );
